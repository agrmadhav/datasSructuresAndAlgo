...
public: 
    ...
    void unionByRank(int u, int v) {
        ...
        if(rank[ulp_u] < rank[ulp_v]) {
            // smaller one attached to bigger one
            parent[ulp_u] = ulp_v; 
        }
        ...
        // when the rank of both is same we can make anyone parent of anyone and we have to increase the rank of parent
        else {
            parent[ulp_v] = ulp_u; 
            rank[ulp_u]++; 
        }
    ...
    void unionBySize(int u, int v) {
        ...
        if(size[ulp_u] < size[ulp_v]) {
            parent[ulp_u] = ulp_v; // u attached tov
            size[ulp_v] += size[ulp_u]; 
        }

        else {
            parent[ulp_v] = ulp_u; // v attatched to u
